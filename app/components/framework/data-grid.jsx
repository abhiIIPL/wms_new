...
export const DataGrid = forwardRef(function DataGrid({
  ...
  // ✅ NEW: PAGINATION-AWARE Ctrl+A handler
  const handleSelectAll = useCallback(() => {
    ...
    console.log('🔥 Current page data items:', data.length);
    
    // ✅ PAGINATION-AWARE: Get current page item IDs
    const currentPageItemIds = data.map(item => item.id);
    console.log('🔥 Current page item IDs:', currentPageItemIds);
    
    // ✅ PAGINATION-AWARE: Check how many items on current page are selected
    const currentPageSelectedIds = currentSelectedIds.current.filter(id => currentPageItemIds.includes(id));
    console.log('🔥 Current page selected IDs:', currentPageSelectedIds);
    
    // ✅ PAGINATION-AWARE: Check if ALL items on current page are selected
    const allCurrentPageSelected = currentPageItemIds.length > 0 && 
                                   currentPageSelectedIds.length === currentPageItemIds.length;
    
    console.log('🔥 All current page selected?', allCurrentPageSelected);
    
    let newSelectedIds;
    
    if (allCurrentPageSelected) {
      // ✅ All items on current page are selected - DESELECT current page items
      console.log('🔥 Deselecting all items on current page');
      newSelectedIds = currentSelectedIds.current.filter(id => !currentPageItemIds.includes(id));
    } else {
      // ✅ Not all items on current page are selected - SELECT all current page items
      console.log('🔥 Selecting all items on current page');
      // Keep selections from other pages and add all current page items
      const selectionsFromOtherPages = currentSelectedIds.current.filter(id => !currentPageItemIds.includes(id));
      newSelectedIds = [...selectionsFromOtherPages, ...currentPageItemIds];
    }
    ...
    if (gridRef.current?.api) {
      ...
      // Get all row nodes on current page
      const allNodes = [];
  ...
  // ✅ NEW: Handle keyboard navigation including Ctrl+A
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      // ✅ NEW: Handle Ctrl+A for select all (only if checkboxes are enabled)
